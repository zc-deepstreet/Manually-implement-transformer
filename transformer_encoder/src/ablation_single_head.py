...
BASE_CONFIG = {
    'd_model': 128,
    'num_heads': 4,  # 注意：配置中保持4，但模型中会改为1
    'd_ff': 512,
    'num_layers': 2,
    'dropout': 0.1,
    'num_epochs': 20,
...
class SingleHeadTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, num_heads, d_ff, num_layers, max_seq_len, dropout=0.1, pad_idx=0):
        ...
        # Transformer layers - 使用单头注意力
        self.layers = nn.ModuleList([
            self._create_single_head_layer(d_model, d_ff, dropout)
    ...
    def _create_single_head_layer(self, d_model, d_ff, dropout):
        """创建单头注意力的编码器层"""
        # 导入必要的组件
        from model import MultiHeadAttention, PositionWiseFFN, ResidualConnection

        class SingleHeadEncoderLayer(nn.Module):
            def __init__(self, d_model, d_ff, dropout):
                super(SingleHeadEncoderLayer, self).__init__()
                # 强制使用单头注意力
                self.self_attention = MultiHeadAttention(d_model, num_heads=1, dropout=dropout)
        ...
        return SingleHeadEncoderLayer(d_model, d_ff, dropout)
...
class AblationTrainer:
    def __init__(self, config, ablation_name, ablation_description):
        ...
        # Initialize ablation model - 强制使用单头
        self.model = SingleHeadTransformer(
...
def compare_with_baseline(ablation_results):
    ...
    try:
        ...
        if performance_drop > 50:
            print("🔴 结论: 多头注意力是Transformer的关键组件!")
        elif performance_drop > 20:
            print("🟡 结论: 多头注意力对性能有显著影响")
        else:
            print("🟢 结论: 多头注意力的影响相对较小")
...
def run_single_head_ablation():
    ...
    trainer = AblationTrainer(
        BASE_CONFIG,
        'single_head',
        '单头注意力的Transformer (4头→1头)'
    )
    trainer.train()
    results = trainer.get_results()

    # 绘制和保存结果
    plot_training_curves(trainer, 'single_head')
    save_results(results, 'single_head')

    # 与基线比较
    compare_with_baseline(results)

    print("\n" + "=" * 60)
    print("下一步：运行无残差连接消融实验")
    print("请输入以下命令:")
    print("python src/ablation_no_residual.py")
    print("=" * 60)
