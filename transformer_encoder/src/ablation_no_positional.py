...
class NoPositionalTransformer(nn.Module):
    """无位置编码的Transformer"""

    def __init__(self, vocab_size, d_model, num_heads, d_ff, num_layers, max_seq_len, dropout=0.1, pad_idx=0):
        super(NoPositionalTransformer, self).__init__()
        self.d_model = d_model
        self.pad_idx = pad_idx

        # Embedding layers - 没有位置编码
        self.token_embedding = nn.Embedding(vocab_size, d_model, padding_idx=pad_idx)
        self.dropout = nn.Dropout(dropout)

        # Transformer layers (复用原始实现)
...
def compare_with_baseline(ablation_results):
    ...
    try:
        ...
        if performance_drop > 50:
            print("🔴 结论: 位置编码是Transformer的关键组件!")
        elif performance_drop > 20:
            print("🟡 结论: 位置编码对性能有显著影响")
        else:
            print("🟢 结论: 位置编码的影响相对较小")

    except FileNotFoundError:
        print("警告: 未找到基线模型结果，请先运行基线模型")


def run_no_positional_ablation():
    """运行无位置编码消融实验"""
    print("=" * 60)
    print("步骤 2: 无位置编码消融实验")
    print("=" * 60)
    ...
    # 绘制和保存结果
    plot_training_curves(trainer, 'no_positional')
    save_results(results, 'no_positional')

    # 与基线比较
    compare_with_baseline(results)

    print("\n" + "=" * 60)
    print("下一步：运行无多头注意力消融实验")
    print("请输入以下命令:")
    print("python src/ablation_single_head.py")
    print("=" * 60)
